...
export default function BudgetChooserScreen() {
  ...
  const workspaceRow = (workspace: Workspace, personal = false) => (
    <Pressable
      ...
      accessibilityLabel={`Open ${personal ? 'My Budget' : workspace.name}`}
      disabled={selectWorkspace.isPending || creating}
      onPress={() => void chooseWorkspace(workspace)}
      style={({ pressed }) => [styles.workspace, { backgroundColor: colors.card, borderColor: colors.border }, pressed && styles.pressed]}
    >
      {personal ? (
      ...
      {selectWorkspace.isPending ? <ActivityIndicator color={colors.primary} /> : <Feather name="chevron-right" size={20} color={colors.mutedForeground} />}
    </Pressable>
  );
  ...
  return (
    <View style={[styles.page, { backgroundColor: colors.background, paddingTop: insets.top + 20 }]}>
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 28 }]} showsVerticalScrollIndicator={false}>
        ...
        <Text style={[styles.eyebrow, { color: colors.brandTeal }]}>JAMVI WORKSPACES</Text>
        <Text style={[styles.title, { color: colors.foreground }]}>Welcome, {displayName(user)}</Text>
        <Text style={[styles.intro, { color: colors.mutedForeground }]}>Choose where you’d like to budget. Personal and shared budgets are always kept separate.</Text>
        ...
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>PERSONAL</Text>
        {loadingWorkspaces ? <ActivityIndicator color={colors.primary} style={styles.loader} /> : privateWorkspace ? workspaceRow(privateWorkspace, true) : (
          <Text style={[styles.empty, { color: colors.mutedForeground }]}>Your personal budget is being prepared. Try again in a moment.</Text>
        )}

        <View style={styles.sectionHead}><Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>SHARED BUDGETS</Text><Pressable accessibilityRole="button" accessibilityLabel="Create a shared budget" onPress={() => { setError(null); setCreating(true); }}><Feather name="plus-circle" size={22} color={colors.primary} /></Pressable></View>
        ...
        <View style={[styles.explanation, { backgroundColor: colors.accent }]}><Feather name="shield" size={18} color={colors.accentForeground} /><Text style={[styles.explanationText, { color: colors.accentForeground }]}>My Budget is private. Shared budgets only show information added by their members.</Text></View>
        <Pressable testID="join-invite" accessibilityRole="button" onPress={() => { setError(null); setJoining(true); }} style={[styles.secondaryButton, { borderColor: colors.border }]}><Feather name="link" size={18} color={colors.primary} /><Text style={[styles.secondaryText, { color: colors.primary }]}>Join with an invitation link</Text></Pressable>
      </ScrollView>
...
const styles = StyleSheet.create({
  page: { flex: 1 }, content: { paddingHorizontal: 20, gap: 12 }, mark: { width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, eyebrow: { fontSize: 12, fontWeight: '700', letterSpacing: 1 }, title: { fontSize: 27, fontWeight: '700' }, intro: { fontSize: 15, lineHeight: 22, marginBottom: 10 }, sectionLabel: { fontSize: 11, fontWeight: '700', letterSpacing: .9, marginTop: 8 }, sectionHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }, workspace: { minHeight: 72, borderWidth: 1, borderRadius: 12, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 12 }, pressed: { opacity: .72 }, workspaceIcon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, workspacePhoto: { width: 40, height: 40, borderRadius: 12, borderWidth: 2 }, workspaceEmoji: { fontSize: 20 }, workspaceText: { flex: 1 }, workspaceTitle: { fontSize: 16, fontWeight: '600' }, workspaceDetail: { fontSize: 13, marginTop: 3 }, loader: { marginVertical: 22 }, empty: { fontSize: 14, lineHeight: 20, paddingVertical: 8 }, explanation: { flexDirection: 'row', gap: 10, borderRadius: 12, padding: 14, marginTop: 12 }, explanationText: { flex: 1, fontSize: 13, lineHeight: 19 }, error: { flexDirection: 'row', gap: 8, padding: 12, borderRadius: 10 }, errorText: { flex: 1, fontSize: 13, lineHeight: 18 }, secondaryButton: { height: 48, borderWidth: 1, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginTop: 4 }, secondaryText: { fontSize: 15, fontWeight: '600' }, scrim: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(1, 28, 78, 0.48)' }, modal: { maxHeight: '88%', borderTopWidth: 1, borderRadius: 20, padding: 20, gap: 12 }, modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, modalTitle: { fontSize: 20, fontWeight: '700' }, modalCopy: { fontSize: 14, lineHeight: 20 }, input: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 13, height: 48, fontSize: 16 }, kind: { borderWidth: 1, borderRadius: 10, padding: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }, kindTitle: { fontSize: 14, fontWeight: '600' }, kindDescription: { fontSize: 12, lineHeight: 16, maxWidth: 265, marginTop: 2 }, primaryButton: { height: 50, alignItems: 'center', justifyContent: 'center', borderRadius: 12, marginTop: 4 }, primaryText: { fontSize: 16, fontWeight: '700' }, disabled: { opacity: .6 },
});