 ...
 *
 * A day's spending arrives as several receipts that make one posting — 1,200
 * for the shopping, 800 at the chemist, 450 for the matatu home. Working that
 * out means leaving the app for a calculator and coming back with a number,
 * losing the sitting on the way. Typing it as it was spent is faster and
 * leaves the arithmetic visible, so a wrong figure can be seen rather than
 * merely disagreed with.
 *
 * Deliberately not eval: this parses text a person typed, and eval on user
 * input in a shipped app is never worth the few lines it saves.
 *
 * No unary minus. An amount is never negative, and refusing "-500" outright is
 * clearer than accepting it and rejecting it a step later. Subtraction between
 * two numbers — "5000-1100" — is ordinary and supported.
 */
