 ...
 * leaves the phone so a treasurer typing forty names at a meeting is told at
 * the input rather than by a failed save.
 *
 * The server enforces the same rule in `api-server/src/lib/contributor-name.ts`
 * and is the authority; this is a mirror, kept honest by lib/__tests__/contributorName.test.ts.
 */
