...
export function createMonthlyReportPdf(data: MonthlyReportPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const header = (continued = false) => {
      ...
      document.fillColor("#FFFFFF").font("Helvetica-Bold").fontSize(19).text("BAJETI", SIDE_MARGIN, 25);
    ...
    document.font("Helvetica").fontSize(7.5).fillColor("#738279").text(
      "Generated by Bajeti from the active group’s data. Amounts are shown in Kenyan shillings (KES).",
      SIDE_MARGIN,
      y,
