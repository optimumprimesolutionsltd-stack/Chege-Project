...
export type ContributionReportPdfData = {
  ...
  /** Public page that shows these figures live, printed in the footer so a
   *  recipient can check the report was not altered after it left Jamvi. */
  verifyUrl?: string;
};
...
export function createContributionReportPdf(data: ContributionReportPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const document = new PDFDocument({
      ...
      // Opens with no password, but is marked read-only: standard readers will
      // not let a recipient edit the figures. A random owner password nobody
      // needs to keep is what turns the permission flags on. This is a
      // deterrent — the verify link in the footer is the real check.
    ...
    y += 20;
    ensureRoom(data.verifyUrl ? 46 : 30);
    if (data.verifyUrl) {
      document.font("Helvetica-Bold").fontSize(8).fillColor("#0A7A54").text(
        "Check this report is genuine",
        SIDE_MARGIN,
        y,
        { width: CONTENT_WIDTH, align: "center" },
      );
      y += 12;
      document.font("Helvetica").fontSize(8).fillColor("#31584A").text(
        data.verifyUrl,
        SIDE_MARGIN,
        y,
        { width: CONTENT_WIDTH, align: "center", link: data.verifyUrl, underline: true },
      );
      y += 16;
    }
    document.font("Helvetica").fontSize(7.5).fillColor("#738279").text(
      "Generated by Jamvi from the group’s records. Amounts are shown in Kenyan shillings (KES). The page above always shows the current figures — if they differ, this report was changed after it was made.",
      SIDE_MARGIN,
      y,
      { width: CONTENT_WIDTH, align: "center" },
    );
