...
describe("Other expense category", () => {
  it("offers Other and asks for a brief explanation in the dashboard quick log", () => {
    ...
    expect(dashboardSource).toContain('title: "Brief description required"');
  ...
  it("offers Other and asks for a brief explanation in the full web form", () => {
    ...
    expect(expensesSource).toContain('title: "Brief description required"');
  ...
  it("offers the same Other-category description path on mobile", () => {
    expect(mobileSource).toContain("...(categories.some((item) => item.name.trim().toLocaleLowerCase() === 'other') ? [] : ['Other'])");
    expect(mobileSource).toContain("category.trim().toLocaleLowerCase() === 'other' && description.trim().length < 3");
