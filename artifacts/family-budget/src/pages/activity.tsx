              ...
              <Card className="border-none shadow-md"><CardContent className="p-4 pt-5 sm:p-6 sm:pt-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Group contribution total</p>
                 <div className="mt-2 flex flex-col gap-0.5 sm:flex-row sm:items-end sm:gap-2"><p className="font-display text-2xl font-bold sm:text-3xl">{formatKes(totalContributed)}</p><span className="text-sm text-muted-foreground sm:mb-1">of {formatKes(totalTarget)} target</span></div>
                  <div className="mt-5 grid grid-cols-1 gap-2 border-t pt-4 sm:grid-cols-3 sm:gap-3">
                   <div className="min-w-0"><p className="break-words text-xs text-muted-foreground">Income-source expected</p><p className="mt-1 font-display text-lg font-bold">{incomeStreamReport.isLoading ? "Loading…" : incomeStreamReport.isError ? "Unavailable" : formatKes(totalExpectedFromSources)}</p></div>
                   <div className="min-w-0"><p className="break-words text-xs text-muted-foreground">Recorded from sources</p><p className="mt-1 font-display text-lg font-bold text-primary">{incomeStreamReport.isLoading ? "Loading…" : incomeStreamReport.isError ? "Unavailable" : formatKes(recordedFromSources)}</p></div>
                   <div className="min-w-0"><p className="break-words text-xs text-muted-foreground">{sourcePlanBalance < 0 ? "Above income plan" : "Still expected"}</p><p className="mt-1 font-display text-lg font-bold">{incomeStreamReport.isLoading ? "Loading…" : incomeStreamReport.isError ? "Unavailable" : formatKes(Math.abs(sourcePlanBalance))}</p></div>
                </div>
              </CardContent></Card>
              <div className="grid gap-4 md:grid-cols-2">
                {members.map((member) => {
                  ...
                  <Card key={member.userId} className="border-none shadow-md"><CardContent className="space-y-3 p-4 pt-5 sm:p-6 sm:pt-5">
                    ...
                    <div className="grid grid-cols-1 gap-2 text-center text-xs sm:grid-cols-3"><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Contributed</p><p className="mt-1 font-bold">{formatKes(member.contributed)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Spent</p><p className="mt-1 font-bold">{formatKes(member.spent)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Net</p><p className={`mt-1 font-bold ${member.net >= 0 ? "text-primary" : "text-destructive"}`}>{member.net >= 0 ? "+" : ""}{formatKes(member.net)}</p></div></div>
                    <div className="space-y-2 border-t pt-3">
                      ...
                      {incomeStreamReport.isLoading ? <div className="h-16 animate-pulse rounded-xl bg-muted/60" /> : incomeStreamReport.isError ? <p className="rounded-xl bg-destructive/10 px-3 py-2 text-xs text-destructive">We couldn’t load this income-source plan. Refresh to try again.</p> : memberStreams.length === 0 ? <p className="rounded-xl bg-muted/50 px-3 py-2 text-xs text-muted-foreground">No income source has been set for {member.name} yet.</p> : <>
                         <div className="grid grid-cols-1 gap-2 text-center text-xs sm:grid-cols-3"><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Expected</p><p className="mt-1 font-bold">{formatKes(expectedFromSources)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Recorded</p><p className="mt-1 font-bold text-primary">{formatKes(recordedForMember)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">{remainingForMember < 0 ? "Above" : "Remaining"}</p><p className="mt-1 font-bold">{formatKes(Math.abs(remainingForMember))}</p></div></div>
                         <div className="divide-y divide-border/60 rounded-xl border border-border/70 px-3">{memberStreams.map((stream) => { const aboveExpected = stream.remainingBalance < 0; return <div key={stream.incomeSourceId ?? stream.sourceName} className="flex items-start justify-between gap-3 py-2.5"><div className="min-w-0"><p className="truncate text-sm font-semibold">{stream.sourceName}</p><p className="mt-0.5 break-words text-xs text-muted-foreground">Expected {formatKes(stream.expectedMonthlyAmount)} · Recorded {formatKes(stream.total)}</p></div><p className={`shrink-0 text-xs font-semibold ${aboveExpected ? "text-primary" : "text-muted-foreground"}`}>{aboveExpected ? `${formatKes(Math.abs(stream.remainingBalance))} above` : `${formatKes(stream.remainingBalance)} left`}</p></div>; })}</div>
                      </>}
                    </div>
                  </CardContent></Card>
                  );
