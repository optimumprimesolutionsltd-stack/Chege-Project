            ...
            <div className="space-y-4">
              <Card className="border-none shadow-md"><CardContent className="p-4 pt-5 sm:p-6 sm:pt-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Group contribution total</p>
                 ...
                 <div className="mt-5 grid grid-cols-3 gap-2 border-t pt-4 sm:gap-3">
                ...
                {members.map((member) => {
                  ...
                  <Card key={member.userId} className="border-none shadow-md"><CardContent className="space-y-3 p-4 pt-5 sm:p-6 sm:pt-5">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="break-words font-display text-lg font-bold">{member.name}</p><p className="text-xs text-muted-foreground">{member.target == null ? "No monthly target" : `${formatKes(member.target)} monthly target`}</p></div><p className="font-display text-xl font-bold text-primary">{formatKes(member.contributed)}</p></div>
                     <div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Contributed</p><p className="mt-1 font-bold">{formatKes(member.contributed)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Spent</p><p className="mt-1 font-bold">{formatKes(member.spent)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Net</p><p className={`mt-1 font-bold ${member.net >= 0 ? "text-primary" : "text-destructive"}`}>{member.net >= 0 ? "+" : ""}{formatKes(member.net)}</p></div></div>
                    <div className="space-y-2 border-t pt-3">
                      ...
                      {incomeStreamReport.isLoading ? <div className="h-16 animate-pulse rounded-xl bg-muted/60" /> : incomeStreamReport.isError ? <p className="rounded-xl bg-destructive/10 px-3 py-2 text-xs text-destructive">We couldn’t load this income-source plan. Refresh to try again.</p> : memberStreams.length === 0 ? <p className="rounded-xl bg-muted/50 px-3 py-2 text-xs text-muted-foreground">No income source has been set for {member.name} yet.</p> : <>
                         <div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Expected</p><p className="mt-1 font-bold">{formatKes(expectedFromSources)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">Recorded</p><p className="mt-1 font-bold text-primary">{formatKes(recordedForMember)}</p></div><div className="min-w-0 rounded-lg bg-muted p-2"><p className="break-words text-muted-foreground">{remainingForMember < 0 ? "Above" : "Remaining"}</p><p className="mt-1 font-bold">{formatKes(Math.abs(remainingForMember))}</p></div></div>
