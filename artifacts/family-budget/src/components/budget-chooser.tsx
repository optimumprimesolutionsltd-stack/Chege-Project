...
<div className="mt-6 flex items-center justify-between rounded-xl bg-primary/[0.05] px-4 py-3"><span className="text-sm font-semibold text-muted-foreground">Planned total</span><span className="font-display text-xl font-bold text-foreground">KES {totalBudget.toLocaleString("en-KE")}</span></div><div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-muted-foreground">Amounts can be changed later</p><Button type="button" className="h-12 rounded-xl px-6" onClick={() => { try { window.localStorage.setItem(`jamvi:onboarding:category-budgets:${encodeURIComponent(userId)}`, JSON.stringify(categoryBudgets)); } catch { /* Continue even when storage is unavailable. */ } setShowCategoryBudgetSetup(false); }}>Approve budget and continue <ChevronRight className="ml-2 h-4 w-4" /></Button></div></div></div></section></main>
    );
  }
